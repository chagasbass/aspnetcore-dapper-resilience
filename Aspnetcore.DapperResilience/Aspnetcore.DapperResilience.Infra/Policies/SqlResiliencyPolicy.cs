...
namespace Aspnetcore.DapperResilience.Infra.Policies
{
    public static class SqlResiliencyPolicy
    {
        ...
        public static IAsyncPolicy GetSqlResiliencyPolicy(TimeSpan? maxTimeout = null, int transientRetries = 3, int networkRetries = 3)
        {
            ...
            var transientPolicy = Policy.Handle<SqlException>(ex => transientNumbers.Contains(ex.Number))
                ...
                (ex, _, ctx) => ctx.GetLogger()?.LogWarning(ex, "{@Operation} Encountered Transient SqlException. Params:{@Param} Sql:{@Sql}", ctx.OperationKey, ContextHelper.ParamContextKey, ContextHelper.SqlContextKey));
            ...
            var networkPolicy = Policy.Handle<SqlException>(ex => networkingNumbers.Contains(ex.Number))
                ...
                (ex, _, ctx) =>
                {
                    ctx.GetLogger()?.LogWarning(ex, "{@Operation} Encountered a Network Error. Params:{@Param} Sql:{@Sql}", ctx.OperationKey, ContextHelper.ParamContextKey, ContextHelper.SqlContextKey);
            ...
            var constraintPolicy = Policy.Handle<SqlException>(ex => constraintViolationNumbers.Contains(ex.Number))
                ...
                1,
                TimeSpan.MaxValue,
                (ex, _, ctx) => ctx.GetLogger()?.LogError(ex, "{@Operation} Encountered a Constraint Violation. Params:{@Param} Sql:{@Sql}", ctx.OperationKey, ContextHelper.ParamContextKey, ContextHelper.SqlContextKey),
        ctx => { }
                );
